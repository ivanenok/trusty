    ...
    public KalkanOCSPResponse sendRequest(Set<X509Certificate> certs) {
        ...
        try {
            ...
            f.addListener(() -> {
                ...
                r.run();
            });//Код слушателя будет выполняться либо в потоке HTTP клиента, либо в потоке, который вызывает метод addListener (если на момент вызова результат ListenableFuture уже готов)
